...
% Author: Julius Welzel (j.welzel@neurologi.uni-kiel.de)
    ...
    idx_time_P2 = dsearchn(cfg.ERP.time_vec',[t_wins_oi(1,1)]'):dsearchn(cfg.ERP.time_vec',[t_wins_oi(1,2)]');
    idx_time_P3 = dsearchn(cfg.ERP.time_vec',[t_wins_oi(2,1)]'):dsearchn(cfg.ERP.time_vec',[t_wins_oi(2,2)]');
    
    
    
    for s = 1:numel(ERP_all)
        
        erp_time(s,:)   = squeeze(mean(mean(ERP_all(s).ERP(idx_chan,:,~any(ERP_all(s).idx_art(idx_chan,:),1)),3),1));
        erp_map_P2(s,:) = squeeze(mean(mean(ERP_all(s).ERP(:,idx_time_P2,~any(ERP_all(s).idx_art(idx_chan,:),1)),3),2));
        erp_map_P3(s,:) = squeeze(mean(mean(ERP_all(s).ERP(:,idx_time_P3,~any(ERP_all(s).idx_art(idx_chan,:),1)),3),2));
    ...
    figure
    subplot(2,2,[1 2])
    plot(cfg.ERP.time_vec,erp_time,'Color',[0.7 0.7 0.7],'LineWidth',0.003)
    hold on
    pm = plot(cfg.ERP.time_vec,mean(erp_time),'k','LineWidth',2)
